            ...
            if (s != "") 
            {
                ...
                if (event == "telemetry")
                {
                    ...
                    for (int i = 0; i < sensor_fusion.size(); i++)
                    {
                        ...
                        //  predict
                        veh_s += double(prev_size) * dt * veh_speed;

                        double car_dist = veh_s - car_s;
                        int veh_lane = 0;
                        // if vehicle in close proximity
                        if (car_dist > -safe_dist_behind and car_dist < safe_dist_front)
                        {
                            // veh lane
                            if(veh_d > 0 and veh_d <= 4) veh_lane = 0;
                            else if (veh_d > 4 and veh_d <= 8) veh_lane = 1;
                            else if (veh_d > 8 and veh_d <= 12) veh_lane = 2;
                            else continue;
                            // close from left/right
                            if (veh_lane == lane - 1) too_close_left = true;
                            if (veh_lane == lane + 1) too_close_right = true;
                        }
                        if (car_dist < 0 and car_dist > -safe_dist_behind)
                        {
                            too_close_behind = true;
                        }
                        // check gaps from right and left
                        if (veh_lane == lane - 1 and car_dist < dist_left and car_dist > 0)
                        {
                            dist_left = car_dist;
                        }
                        if (veh_lane == lane + 1 and car_dist < dist_right and car_dist > 0)
                        {
                            dist_right = car_dist;
                        }
                        // if vehicle in our lane
                        if (veh_lane == lane)
                        {
                            if (car_dist > 0 and car_dist < safe_dist_front)
                            {
                                too_close = true;
                                ref_speed = veh_speed;
                            }
                    ...
                    if (lane == 0) too_close_left = true;
                    else if (lane == 2) too_close_right = true;

                    // make a decision
                    if (too_close)
                    {
                        if (!too_close_left and !too_close_right)
                        {
                            if (dist_left > dist_right)
                            {
                                lane--;
                                if (too_close_behind) lane++;
                            }
                            else 
                            {
                                lane++;
                        ...
                        else if (!too_close_left and lane != 0)
                        {
                            lane--;
                            if (too_close_behind) lane++;
                        }
                    ...
                    {
                        // try return to center
                        if (lane == 0 and !too_close_right)
                        {
                            lane++;
                            if (too_close_behind) lane--;
                        }
                        if (lane == 2 and !too_close_left)
                        {
                            lane--;
                            if (too_close_behind) lane++;
                        }
